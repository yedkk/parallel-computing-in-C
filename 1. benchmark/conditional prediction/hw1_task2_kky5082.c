...
int main(int argc,char *argv[]){
 int N =1000;
  ...
  for(i=0;i<N;i++){ //initialize arrays
     A[i]=0e0;B[i]=1e0;
     C[i]=(2.0*((double)rand()/(double)RAND_MAX))-1.0;  //initialize with random value
     //C[i]=1.0; //initialize with positive value
     //C[i]=-1.0; //initialize with negative value
     D[i]=3e0;
  }
